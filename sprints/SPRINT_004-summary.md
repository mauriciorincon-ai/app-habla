...
duración conocida y solo camina hacia adelante) · K (**K1–K4 limpias, stopper K5 rematado:**
la "prioridad" que el usuario creyó ver en K1 es la alineación diseñada — el juego usa solo las
palabras de sus temas y el lote las propuso grabar primero; y para llegar a las consignas ya no
...
oíste: N"; motor, celebración final y registro de juicios intactos; y su segundo hallazgo: la
ronda **no tenía puerta de salida** — gemelas no usa el MarcoJuego que trae el "Salir" — ahora
tiene el "Salir" estándar con reinicio completo, regla del bloque G; tercero: **las parejas
crecen de 6 a 12** (contrastes terapéuticos como r/l; pictos ARASAAC revisados uno a uno —
"rama" y "piso" descartados por débiles; banco a 58 palabras) con **mezcla del día** por
...
garantía en positivo: "Este número nunca baja") · N (**4/4 limpias — "visual muy buena, limpia,
minimalista" — con tres remates:** botón del vacío centrado; "ver TODO el rumbo" — la semana
gana cápsulas completadas, vueltas del globo POR intento y segundos totales de voz, con la
regla honesta escrita: el Rumbo cuenta lo que llegó a la celebración; y los **hitos crecen de 7
a 16 posibles** — primeras veces de cada juego, 10 s seguidos y acumulados que nunca bajan —
con los umbrales de palabras quietos en 10/25 porque el universo real son las palabras de SUS
temas y un 50 sería inalcanzable, o sea deshonesto) · O-parcial (O1+O2 con **cuatro remates de
UX**: el objetivo ya no regaña a medio teclazo — motor puro `sugerir.ts` (prefijos + distancia
≤ 2, sin IA) propone términos del vocabulario real ("¿Vas hacia alguna de estas?"); guardar un
error de ortografía **pregunta primero** ("¿Quisiste decir «animales»?", con "guardar tal cual"
como derecho intacto); todo en UNA tarjeta y el botón **nombra lo que guarda** ("Guardar
«animales»"); y en el estudio las grabadas que sirven al objetivo llevan su **diana** — mismo
predicado del lote. **Re-prueba de O1: la eñe sobrevive** — "bañ" sugería «bano» (el motor
mostraba la clave normalizada de comparar) más rana/lana/mano de relleno; ahora `sugerir`
...
(«medio» → «medios»), y el campo activa el corrector nativo del navegador/teclado. **Cuarto
...
verde al alinear y con aviso cuando no. El usuario reprodujo el grano fino (temas
Transporte+Espacio + "animales" salía verde: las cápsulas van por etapa y SÍ alineaban, pero
los dibujos van por temas y eran cero) y pidió colores aptos para su **daltonismo básico** →
`estadoDeAlineacion` v2 con CUATRO estados en tres familias de color + FORMA: verde+diana con
desglose "Alineando ahora: N cápsulas · M dibujos" (los juegos cambian) · azul+círculo-info
("la cápsula sí, los dibujos no — ese tema no está entre los elegidos", con enlace a Ajustes;
también el fuera-de-alcance) · rojo+triángulo ("priorizado, pero la app aún no tiene contenido
de esto"). El chip verde de sugerencias dejó de prometer de más ("se ponen primero en lo que
él ve"). La alerta entre casas que pidió va al backlog (item 9, atada a la sync dos-casas)).
